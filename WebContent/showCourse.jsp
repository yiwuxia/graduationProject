...
<script type="text/javascript">
	...
	function jumps(){
		...
		if(currentPageIndex != ""){
			if(!isNaN(index) & index > 0){
				//当前页面索引是正整数
				//数据传递到服务端
				window.location = "${pageContext.request.contextPath }/Controller?op=showCourse&currentPageIndex=" + parseInt(index) + "&courseNum=" + courseNum;
			}else{
				alert("请输入一个正整数！");
	...
	//查询某一页的课程详细信息
	function showCourse(currentPageIndex){
		var courseNum = document.getElementById("courseNum").value;
		//数据传递到服务端
		window.location = "${pageContext.request.contextPath }/Controller?op=showCourse&courseNum=" + courseNum + "&currentPageIndex=" + currentPageIndex;
	}
</script>
<body>
	<div style="margin-left: 120px;margin-bottom: 10px">
		<label><b>课程详情&nbsp;&nbsp;</b></label>
	</div>
					...
					<table width="100%" id="t2">
						...
						<c:choose>
							<c:when test="${empty page.courseSelectionsList }">
								<td colspan="8" height="50px" align="center">暂时没有数据</td>
							</c:when>
							<c:otherwise>
	...
	<c:choose>
		<c:when test="${page.pageCount != 0}">
			<table style="position:fixed;bottom:20px;left:90px">
				<tr>
					<td>第${page.currentPageIndex }页/共${page.pageCount }页</td>
					<td>
						<a href="javascript:showCourse(1)"><span class="border" style="margin-left: 25px">&laquo;</span></a>
					</td>
					<c:forEach begin="${page.startIndex }" end="${page.endIndex }" var="n">
						<td><a href="javascript:showCourse(${n})"><span class="${n == page.currentPageIndex?'currentPageIndex':'border' }">${n }</span></a></td>
					</c:forEach>
					<td>
						<a href="javascript:showCourse(${page.pageCount})"><span class="border">&raquo;</span></a>
					</td>
					<td>
